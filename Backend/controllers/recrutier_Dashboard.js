...
export const recruiterDashboard = async (req, res) => {
    try {
        ...
        const jobIds = job.map(j => j._id);
        const totalApplication = await Application.countDocuments({ job: { $in: jobIds } });
        res.status(200).json({ totalJobs, totalApplication });
    } catch (err) {
...
export const updateApplicantStatus = async (req, res) => {
    try {
        ...
        // FIXED: was using undefined variable `updated` — now correctly `updatedStatus`
        ...
        res.status(200).json({ message: "Application Updated", updated: updatedStatus });
    } catch (err) {
        res.status(500).json({ message: "Server Error" });
    }
};
